...
- Standard targets that should be present:
  ...
  - `make clean` - Remove extra artifacts
  - `make increment-patch` - Increment the static version patch number
  - `make increment-minor` - Increment the static version minor number
  - `make increment-major` - Increment the static version major number
  - `make push` - Tag and push the github release
- Add project-specific utility targets as needed (e.g., `make docker`, `make migrate`, `make generate`)
- No need for a "help" target
- Default should always be the build step
...
## README.md
- The README.md file is intended for user facing project information and instructions.
- README.md file must be kept up to date with the base design at all times
- Keep the README.md succinct

## DESIGN.md
- The DESIGN.md file is intended for full project requirements, UI and architecture
- DESIGN.md should be kept in sync with the project state at all times.
- DESIGN.md should be deep, with full detail included.

## Git and Version Control
- Keep `CHANGELOG.md` current for notable changes; it is **not** enforced by `make push`
- **MANDATORY**: Never run `git add`, `git commit`, or `git push` manually
...
**CRITICAL: All changes must be documented in README.md and DESIGN.md**
- After making any significant changes to the codebase, update the README.md and DESIGN.md file
- Document new features, bug fixes, configuration changes, and dependency updates
- Include a "Changelog" or "Updates" section in the README.md (brief)
- Update installation instructions if dependencies change
- Update usage examples if functionality changes
- Keep the README.md and DESIGN.md current and comprehensive
